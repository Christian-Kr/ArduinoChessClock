...
/*!
  @brief Create a TFTSevenSegmentModule represent a seven segment module than can display one digit
   @param tft       pointer to Elegoo_TFTLCD
...
*/
TFTSevenSegmentModule::TFTSevenSegmentModule(Elegoo_TFTLCD* tft, int16_t x = 0, int16_t y = 0, int16_t w = 16, int16_t h = 32, uint16_t onColor = 0, uint16_t offColor = 0, int16_t ledWidth = 3, boolean on = true)
...
void TFTSevenSegmentModule::draw_F_LeftUpperLed(uint16_t color) {
  for (int i = 0; i < m_ledWidth; i++) {
    m_tft->drawFastVLine(m_x + i, m_y + i, m_h / 2 - 2 * i, color);
  }
}
...
void TFTSevenSegmentModule::draw_E_LeftBottomLed(uint16_t color) {
  for (int i = 0; i < m_ledWidth; i++) {
    m_tft->drawFastVLine(m_x + i, m_h / 2 + m_y + i + 1, m_h / 2 - 2 * i, color);
  }
}
...
void TFTSevenSegmentModule::draw_B_RightUpperLed(uint16_t color) {
  for (int i = 0; i < m_ledWidth; i++) {
    m_tft->drawFastVLine(m_x + m_w - i, m_y + i, m_h / 2 - 2 * i, color);
  }
}
...
void TFTSevenSegmentModule::draw_C_RightBottomLed(uint16_t color) {
  for (int i = 0; i < m_ledWidth; i++) {
    m_tft->drawFastVLine(m_x + m_w - i, m_y + m_h / 2 + i + 1, m_h / 2 - 2 * i, color);
  }
}
...
void TFTSevenSegmentModule::draw_G_MiddleLed(uint16_t color) {
  if (m_ledWidth < 2) {
    m_tft->drawFastHLine(m_x+1, m_y + m_h / 2, m_w, color);
  } else {
    int ledWidth = m_ledWidth < 2 ? 1 : m_ledWidth / 2;
    for (int i = 0; i < ledWidth + m_ledWidth % 2; i++) {
      m_tft->drawFastHLine(m_x + i + 2, m_y + m_h / 2 - i, m_w - 2 * i - 4, color);
      if (m_ledWidth > 1) {
        m_tft->drawFastHLine(m_x + i + 2, m_y + m_h / 2 + i + 1, m_w - 2 * i - 4, color);
      }
    }
  }
}
...
void TFTSevenSegmentModule::draw_A_UpperLed(uint16_t color) {
  if (m_ledWidth < 2) {
    m_tft->drawFastHLine(m_x , m_y , m_w , color);
  } else {
    for (int i = 0; i < m_ledWidth; i++) {
      m_tft->drawFastHLine(m_x + i + 3, m_y + i, m_w - 2 * i - 5, color);
    }
  }
...
void TFTSevenSegmentModule::draw_D_BottomLed(uint16_t color) {
  if (m_ledWidth < 2) {
    m_tft->drawFastHLine(m_x, m_y + m_h , m_w, color);
  } else {
    for (int i = 0; i < m_ledWidth; i++) {
      m_tft->drawFastHLine(m_x + i + 3, m_y + m_h - i, m_w - 2 * i - 5, color);
    }
  }
}
...
/*!
  @brief    Change Elegoo_TFTLCD instance to display on
  @param tft                pointer to Elegoo_TFTLCD
*/
void TFTSevenSegmentModule::setTft(Elegoo_TFTLCD* tft) {
  m_tft = tft;
}
